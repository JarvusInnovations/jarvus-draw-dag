...
*/
Ext.define('Jarvus.draw.layout.DagLayout', {
    alternateClassName: 'dagfn',
    singleton: true,
     ...
     *
     */
    compute_successors: function(dag) {
        var successors = {};
        for (nn in dag.nodes) {
            successors[nn] = [];
        }
     ...
     *
     */
    longestPathLayering: function(dag) {
     ...
     * }
     */
    addDummyNodes: function(dag, layers) {
        var layer_by_node = {};
     ...
     * }
     */
    two_layer_adjacent_exchange: function(nodes, successors,
            layer1, layer2) {
        var crossings = dagfn.compute_crossing_numbers(successors, layer1,
                layer2);
        var cn = dagfn.compute_crossing(crossings, layer2);
        var lb = dagfn.compute_crossing_lower_bound(crossings, layer2);
        ...
        while (true) {
            ...
            // compute
            cross = dagfn.compute_crossing(crossings, l2);
     ...
     * }
     */
    topDownLayerByLayerSweep: function(dag, layers_orig, two_layer_algorithm) {
         var successors = dagfn.compute_successors(dag);
     ...
     *
     */
    bottomUpLayerByLayerSweep: function(dag, layers, two_layer_algorithm) {
         ...
         var res = dagfn.topDownLayerByLayerSweep(reversed_dag,
     ...
     *
     */
    orderLayersUsing2waysSweep: function(dag, layers, algo) {

        var res_top = dagfn.topDownLayerByLayerSweep(dag, layers, algo);
        var res_up = dagfn.bottomUpLayerByLayerSweep(dag, layers, algo);
     ...
     *
     */
    compute_crossing: function(matrix, layer2) {
     ...
     *
     */
    compute_crossing_lower_bound: function(matrix, layer2) {
        var n2 = layer2.length;
        var lb = 0;
        for (var i = 0; i < n2; ++i) {
            for (var j = i + 1; j < n2; ++j)
     ...
     *
     */
    compute_crossing_numbers: function(successors, layer1, layer2) {
